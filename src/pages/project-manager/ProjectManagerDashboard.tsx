...
export default function ProjectManagerDashboard() {
  ...
  const { data: stats, isLoading } = useQuery<Stats>({
    ...
    queryFn: async () => {
      ...
      const inUseCount = allStatuses.filter((s) => s === "deployed").length;
  ...
  const actions = [
    {
      to: "/project-manager/equipments",
      icon: <List className="w-5 h-5" />,
      label: "All Equipment",
      desc: "View inventory",
    },
    {
      to: "/project-manager/deployed",
      icon: <ClipboardList className="w-5 h-5" />,
      label: "Deployed",
      desc: "Currently in use",
    },
    {
      to: "/project-manager/requests",
      icon: <Clock className="w-5 h-5" />,
      label: "Requests",
      desc: "Equipment requests",
    },
    {
      to: "/project-manager/allocate",
      icon: <PackagePlus className="w-5 h-5" />,
      label: "Allocate",
      desc: "Assign equipment",
    },
    {
      to: "/project-manager/team",
      icon: <Users className="w-5 h-5" />,
      label: "Team",
      desc: "Assignments & allocations",
    },
    {
      to: "/project-manager/reports",
          ...
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-8">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-4 max-w-md font-normal">
                Track equipment allocation and project assignments
              </p>
            </div>
          </div>
        </header>
