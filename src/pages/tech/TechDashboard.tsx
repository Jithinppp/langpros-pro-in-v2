...
export default function TechDashboard() {
  ...
  const { data: stats, isLoading } = useQuery<Stats>({
    ...
    queryFn: async () => {
      ...
      const inProgressCount = allStatuses.filter(
        (s) => s === "maintenance",
      ).length;
  ...
  const roleTitle = user?.email?.split("@")[0] || "Tech";
  ...
  const actions = [
    {
      ...
      label: "Repairs",
      desc: "Manage repair jobs",
    },
    {
      to: "/tech/maintenance",
      icon: <Settings className="w-5 h-5" />,
      label: "Maintenance",
      desc: "Scheduled servicing",
    },
    {
      to: "/tech/pending",
    ...
    },
    {
      ...
      label: "Completed",
      desc: "Finished work orders",
    },
    {
      to: "/tech/issues",
      icon: <AlertTriangle className="w-5 h-5" />,
      label: "Issues",
      desc: "Reported problems",
    },
    {
      to: "/tech/reports",
          ...
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-8">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-4 max-w-md font-normal">
                Manage repairs and maintenance tasks
              </p>
            </div>
          </div>
        </header>
